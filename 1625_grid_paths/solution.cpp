...
int solve(const string &moves, bool curr_path[board_size * board_size - 1], int move_index, int x, int y);
...
int try_move(const string &moves, bool curr_path[board_size * board_size - 1], int move_index, int x, int y, char move)
{
    int num_solutions = 0;
    switch (move)
    {
...
int solve(const string &moves, bool curr_path[board_size * board_size - 1], int move_index, int x, int y)
{
    ...
    else
    {
        if (moves[move_index] == '?')
        {
            num_solutions += try_move(moves, curr_path, move_index, x, y, 'U');
            num_solutions += try_move(moves, curr_path, move_index, x, y, 'D');
            num_solutions += try_move(moves, curr_path, move_index, x, y, 'L');
            num_solutions += try_move(moves, curr_path, move_index, x, y, 'R');
        }
